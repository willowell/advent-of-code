...
import scala.collection.mutable.{ArrayBuffer, ArrayDeque, HashMap, HashSet, PriorityQueue, Queue}
...
def bfs2[S](
  ...
  goalTestFn: GoalTestFn[S],
  getSuccessorsFn: GetSuccessorsFn[S],
  isValidNeighbour: (S, S) => Boolean,
): Option[(Node[S], Int)] = {
  // Frontier is where we've yet to go.
  var frontier = Queue[Node[S]](Node(initial, None))

  // Explored is where we've been.
  var explored = HashMap[S, Int](initial -> 0)
  ...
  while (frontier.nonEmpty) {
    ...
    // If the current node matches the goal, we're done!
    if (goalTestFn(currentState)) {
      val cost = explored(currentState)

      return Some((currentNode, cost))
    }

    val neighbours = getSuccessorsFn(currentState)

    neighbours.filterNot { explored.contains(_) }.foreach { nextState =>
      if (isValidNeighbour(currentState, nextState)) {
        val _ = frontier.enqueue(Node(nextState, Some(currentNode)))

        val _ = explored.put(nextState, (explored(currentState) + 1))
      }
    }
  }
...
def astar[S](
  ...
  while (!frontier.isEmpty) {
    ...
    for (succ <- getSuccessorsFn(currentState)) {
      ...
      val newCost = currentNode.cost + costFn(currentState, succ)
